...
- Keep page copy data-driven. Prefer updates in `portfolioData` over embedding new strings directly in the page unless the text is page-structural.

## Home Page Specifics

- `Home.tsx` owns the fixed `Canvas`, `ScrollControls`, `<Scroll html>`, section order, and `pages` measurement.
...
- `ProjectDetail.tsx` derives the scene, accent colors, and next-project CTA from `portfolioData.projects`.
...
- `npm run lint`
- `npm run build`
- Manual route smoke check for `/` and any affected `/project/:id` routes
